...
export const get_blog_list_category = (slug) => async dispatch => {
    ...
    try{
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/blog/list/by_category?slug=${slug}`, config)
        if (res.status === 200) {
            dispatch({
...
export const get_blog_list_category_page = (slug, page) => async dispatch => {
    ...
    try{
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/blog/list/by_category?slug=${slug}&p=${page}`, config)
        if (res.status === 200) {
            dispatch({
                type: GET_BLOG_LIST_CATEGORIES_SUCCESS,
                payload: res.data
